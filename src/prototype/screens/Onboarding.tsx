...
export function Onboarding({ v }: { v: ViewModel }) {
  return (
    <>
      <div style={S('position:absolute;inset:0;display:flex;flex-direction:column;background:#F4F2ED')}>
          ...
          <>
            <div style={S('padding:60px 22px 0;display:flex;align-items:center;gap:12px')}>
              ...
              >
                <div
                  style={S(
                    `height:100%;background:#17A05E;border-radius:99px;transition:width .45s cubic-bezier(.2,.8,.2,1);width:${v.obPct ?? ''}`,
                  )}
                />
              </div>
        ...
        {v.ob0 ? <OnboardingIntro v={v} /> : null}
        {v.obQuestion ? <OnboardingQuestion v={v} /> : null}
        {v.isTargets ? <TargetsScreen v={v} /> : null}
        {v.obBuilding ? <BuildingScreen v={v} /> : null}
      </div>
    </>
  );
}
