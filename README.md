...
`resources/icon.png` and `resources/splash.png` are generated by
`npm run icons`, from the same path geometry as the favicon and the PWA icons —
see `tools/icons/generate.mjs`. `npm run ios:assets` expands them into every
size Xcode wants.
